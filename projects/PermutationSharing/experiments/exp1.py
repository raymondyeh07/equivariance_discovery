"""Experiment for sum of numbers dataset."""
import os
import numpy as np
...
def main():
    ...
    if a_rank == 1:
        w_scale = 1
    if a_rank == 2:
        w_scale = -2  # For rank 2 experiment.
        ...
        for method in method_list:
            for rand_seed in tqdm(range(0, num_runs)):
                ...
                train_set = NumbersSumDataset(
                    num_train, max_length=max_length,
                    A_rank=a_rank, add_noise=add_noise,
                    rand_seed=rand_seed, w_scale=w_scale)
                A_gt = train_set.A
                val_set = NumbersSumDataset(
                    num_val, max_length=max_length, A=A_gt,
                    add_noise=add_noise,
                    rand_seed=rand_seed+num_runs, w_scale=w_scale)
                hyper_set = NumbersSumDataset(
                    num_hyper, max_length=max_length, A=A_gt,
                    add_noise=add_noise,
                    rand_seed=rand_seed+2*num_runs, w_scale=w_scale)
                test_set = NumbersSumDataset(
                    num_test, max_length=max_length, A=A_gt,
                    add_noise=False,
                    rand_seed=rand_seed+3*num_runs, w_scale=w_scale)
                # Train the models.
